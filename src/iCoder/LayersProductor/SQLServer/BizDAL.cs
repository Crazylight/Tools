...
{
	public class BizDAL : CommonPro
	{
		static BizDAL instance;
		public BizDAL(DataTable dt, string projName, string entityName)
			: base(dt, projName, entityName)
		{

		}
		public static BizDAL getInstance(DataTable dt, string projName, string classname, string entityName)
		{
			// if (instance == null)
			{
				instance = new BizDAL(dt, projName, entityName);
			}
			return instance;
		}
		...
		public string CreateDAL()
		{
			...
			sb.Append("\r\n");
			sb.AppendFormat(string.Format("namespace {0}.DAL\r\n", _projName));
			sb.Append("{\r\n");
			//ClassDAL
		...
		private string CreateMethods()
		{
			...
			sb.Append("\t///</summary>\r\n");
			sb.Append(" \tpublic DataTable GetList()\r\n");
			sb.Append("\t\t {\r\n");
			...
			sb.Append("            return   _sqlH.GetDataTable(sql);\r\n");
			sb.Append("         }\r\n\r\n");
			...
			sb.Append("\t///根据条件获取DataTable\r\n");
			sb.Append("\t///</summary>\r\n");
			sb.Append("\tpublic DataTable GetDtById(string Id)\r\n");
			sb.Append("\t{\r\n");
			sb.Append("\t\tstring sql = \"SELECT ");
			foreach (DataRow dr in _dt.Rows)
			{
			...
			#endregion
			#region GetDataTableByIds
			sb.Append("\t///<summary>\r\n");
			sb.Append("\t///根据ID获取DataTable\r\n");
			sb.Append("\t///</summary>\r\n");
			sb.Append(" \tpublic DataTable Get" + _entityName + "DtByIds(string ids)\r\n");
			sb.Append("         {\r\n");
			sb.Append("string sql = \"SELECT ");
			foreach (DataRow dr in _dt.Rows)
			{
			...
			sb.Remove(sb.Length - 2, 2);
			sb.Append(" FROM [" + _dt.TableName + "]  WHERE " + _dt.Rows[0]["name"].ToString().ToUpper() + " IN( \" + ids + \")\";\r\n\r\n");
			sb.Append("  return  _sqlH.GetDataTable(sql);\r\n");
			sb.Append("         }\r\n\r\n");
			#endregion
			...
			sb.Append("            return _sqlH.GetDataTable(sql, start_index, count);\r\n");
			sb.Append("         }\r\n\r\n");
			...
			sb.Append("\t///根据条件获取DataTable\r\n");
			sb.Append("\t///</summary>\r\n");
			sb.Append(" \tpublic DataTable GetExportDtByConditions(string conditions)\r\n");
			sb.Append("         {\r\n");
			sb.Append("    string sql = \"SELECT ");
			foreach (DataRow dr in _dt.Rows)
			{
			...
			sb.Append(")\r\n         {\r\n");
			sb.Append("string sql = \"SELECT COUNT(1)");
			sb.Append(" FROM [" + _dt.TableName + "]  WHERE 1=1 \" + conditions;\r\n");
			sb.Append("  return _sqlH.GetScalar<int>(sql);\r\n");
			sb.Append("         }\r\n\r\n");
			#endregion
			#endregion
			...
			sb.Append("\t///删除特定ID的记录\r\n");
			sb.Append("\t///</summary>\r\n");
			sb.Append(" \tpublic void DelById(int id)\r\n");
			sb.Append("         {\r\n");
			sb.Append("//string sql = \"DELETE FROM [" + _dt.TableName + "]  WHERE ID = \" + id" + ";\r\n");
			sb.Append("string sql = \"UPDATE " + _dt.TableName + " SET STATUS = -1 WHERE ID = \" + id" + ";\r\n");
			...
			sb.Append("\t///</summary>\r\n");
			sb.Append(" \tpublic void Del" + _entityName + "ByIds(string ids)\r\n");
			sb.Append("         {\r\n");
			...
			#region IsUsed
			sb.Append("\t///<summary>\r\n");
			sb.Append("\t///判断对应ID的记录是否在用\r\n");
			sb.Append("\t///</summary>\r\n");
			sb.Append(" \tpublic bool IsUsed(int id)\r\n");
			sb.Append("         {\r\n");
			sb.Append(" //所有相关表都要判断一下\r\n");
			sb.Append(" string sql = \"SELECT COUNT(1) FROM ?? WHERE ?= \" + id" + ";\r\n");
			sb.Append(" return  _sqlH.GetScalar<int>(sql) > 0;\r\n");
			sb.Append("         }\r\n\r\n");
			#endregion
			...
			sb.Append("\t///更新状态\r\n");
			sb.Append("\t///</summary>\r\n");
			sb.Append(" \tpublic void SetStatus(string id, string status)\r\n");
			sb.Append("         {\r\n");
			sb.Append("_sqlH.Execute(\"UPDATE " + _dt.TableName + " SET STATUS = '\"+ status +\"' WHERE ID = '\" + id + \"';\r\n");
			...
			sb.Append("\t///获取对应ID的实体\r\n");
			sb.Append("\t///</summary>\r\n");
			sb.Append("\tpublic " + _entityName + "Entity GetEntityById(string id)\r\n");
			sb.Append("        {\r\n");
			sb.Append("            " + _entityName + "Entity result = new " + _entityName + "Entity();\r\n");
			sb.Append("           string sql = \"SELECT ");
			foreach (DataRow dr in _dt.Rows)
			{
				sb.Append(dr["name"].ToString().ToUpper() + ", ");
			...
			sb.Append("\t///</summary>\r\n");
			sb.Append("\tpublic  ObservableCollection<" + _entityName + ">  Get" + _entityName + "EntityList()\r\n");
			sb.Append("        {\r\n");
			...
			//函数体
			sb.Append("         private void ReadTo" + _entityName + "Entity(SqlDataReader dr, " + _entityName + "Entity dol)\r\n");
			sb.Append("         {\r\n");
		...
		private string Insert()
		{
			...
			insert.Append("\t\t\t///</summary>\r\n");
			insert.Append("public int Insert(");
			for (int i = 0; i < _dt.Rows.Count; i++)
			{
				if (_dt.Rows[i]["name"].ToString().ToLower().Equals("id")
					...
					|| _dt.Rows[i]["is_identity"].ToString().ToLower().Equals("1"))
					continue;
			...
			insert.Append("           return _sqlH.GetScalar<int>(sql);\r\n");
			insert.Append("  }\r\n\r\n");
			#endregion
		...
		private string Update()
		{
			...
			sb.Append("\t///更新记录\r\n");
			sb.Append("\t///</summary>\r\n");
			sb.Append("      public void Update(string id, ");
			StringBuilder param = new StringBuilder();
			foreach (DataRow dr in _dt.Rows)
			{
				if (dr["name"].ToString().ToLower().Equals("id")
				   || dr["name"].ToString().ToLower().Equals("addtime")
				   || dr["name"].ToString().ToLower().Equals("adder")
				   || dr["name"].ToString().ToLower().Equals("adddate"))
					continue;
			...
			sb.Append("      {\r\n");
			//sb.Append("  //重复的不能保存， 判断重复时，应该排除掉自己\r\n");
			//sb.Append("if(_sqlH.GetScalar<int>(\"SELECT COUNT(1) FROM " + classname + " WHERE ID <> '\" + " + dt.Rows[0]["name"].ToString().ToLower() + "+ \"' AND " + dt.Rows[1]["name"].ToString() + " = '\" + " + dt.Rows[1]["name"].ToString().ToLower() + " + \"'\") > 0)\r\n");
			//sb.Append("{\r\n");
			//sb.Append("   return -1;\r\n");
			//sb.Append("}\r\n");
			...
			{
				if (_dt.Rows[i]["name"].ToString().ToLower().Equals("id")
				   || _dt.Rows[i]["name"].ToString().ToLower().Equals("adder")
				   || _dt.Rows[i]["name"].ToString().ToLower().Equals("addtime")
				   || _dt.Rows[i]["name"].ToString().ToLower().Equals("adddate")
					|| _dt.Rows[i]["is_identity"].ToString().ToLower().Equals("1"))
					continue;
		...
		private string InsertDOL()
		{
			...
			sb.Append("\t///插入实体\r\n");
			sb.Append("\t///</summary>\r\n");
			sb.Append("\tpublic int Insert(" + _entityName + "Entity dol)\r\n");
			sb.Append("        {\r\n");
			sb.Append("           string sql =");
			StringBuilder insert = new StringBuilder();
			...
			for (int i = 0; i < _dt.Rows.Count; i++)
			{
				...
				switch (ConvertMsSqlToDoNetTypeNotNull(dr))
				{
					...
					case "DateTime?":
						...
						// values.Append("'\" + dol." + dt.Rows[i]["name"].ToString() + "+ \"', ");
						break;
					default:
						values.Append("\" + dol." + _dt.Rows[i]["name"].ToString() + "+ \", ");
						break;
		...
		private string UpdateDOL()
		{
			...
			sb.Append("\t///更新记录\r\n");
			sb.Append("\t///</summary>\r\n");
			sb.Append("      public void Update(" + _entityName + "Entity dol)\r\n");
			sb.Append("      {\r\n");
			sb.Append("          string sql =");
			...
			{
				if (_dt.Rows[i]["name"].ToString().ToLower().Equals("id")
					|| _dt.Rows[i]["name"].ToString().ToLower().Equals("adderid")
					|| _dt.Rows[i]["name"].ToString().ToLower().Equals("addername")
					|| _dt.Rows[i]["name"].ToString().ToLower().Equals("addid")
					|| _dt.Rows[i]["name"].ToString().ToLower().Equals("addname")
					|| _dt.Rows[i]["name"].ToString().ToLower().Equals("addtime")
					|| _dt.Rows[i]["name"].ToString().ToLower().Equals("adddate"))
					continue;
